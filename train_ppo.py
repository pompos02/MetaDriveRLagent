...
class CustomMetricsCallback(BaseCallback):
    def __init__(self, verbose=0):
        ...
        self.current_episode_steps = None  # Set in _on_training_start

    def _on_training_start(self) -> None:
        n_envs = self.training_env.num_envs
        self.current_episode_steps = [0] * n_envs  # one counter per env
...
def make_env():
    return MyEnvNoTraffic()

def make_env_fn(seed_offset):
    def _init():
        env = MyEnvNoTraffic(dict(num_scenarios=4, start_seed=seed_offset))
        return env
    return _init
...
if __name__ == "__main__":
    ...
    env = SubprocVecEnv([lambda: MyEnvNoTraffic(dict(num_scenarios=1, start_seed=0)) for _ in range(8)])
    # env = make_env()
    # Build the PPO model
    model = PPO(
        policy="MlpPolicy",
        ...
        gae_lambda=0.97,
        clip_range=0.2,
        ent_coef = 0.01,      # encourages more exploration initially
